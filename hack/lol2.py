...
SCALE_WIDTH = 250
SCALE_HEIGHT = 40
SCALE_COLOR = (255,0,0,0)
FPS_CLOCK = pygame.time.Clock

teacher_speed_zero_time = 0
...
def show_menu():
    ...
    rules_image = pygame.transform.scale(rules_image, (88, 115))
...
def show_you_won_window():
    ...
    while show_won:
        ...
        # Отображаем  "won_image" поверх белой поверхности
...
while time.time() - start_time < 3:
    ...
    timer_text = font.render(str(int(4 - (time.time() - start_time))), True, WHITE)
    screen.blit(timer_text, (WIDTH // 2-30, HEIGHT // 2-30))
...
while running: 
    ...
    for teacher in proctor_avinash:
        if teacher['direction'] == 'up':
            ...
            # Change direction randomly
                new_direction = random.choice(['right', 'down', 'left'])
                teacher['direction'] = new_direction

            # Change endpoint randomly
            ...
            if teacher['x'] >= WIDTH - teacher_width - 365:
                new_direction = random.choice(['up', 'down', 'left'])
                teacher['direction'] = new_direction
                if new_direction == 'up':
            ...
            if teacher['y'] >= HEIGHT - teacher_height - 0:
                new_direction = random.choice(['up', 'right', 'left'])
                teacher['direction'] = new_direction
                if new_direction == 'up':
                    teacher['image'] = pygame.image.load('photo/avi2.png')
            ...
            teacher['x'] -= teacher['speed']
            if teacher['x'] <= 145:
                new_direction = random.choice(['up', 'right', 'down'])
                teacher['direction'] = new_direction
                if new_direction == 'up':
                    teacher['image'] = pygame.image.load('photo/avi2.png')
                elif new_direction == 'right':
        ...
        if clicked_desk and (
            ...
            teacher['speed']=0
            teacher['image'] = pygame.image.load('photo/avi_r.png')
            
            update_scale = False
                
            pygame.mixer.music.load('sounds/avi.mp3')
            pygame.mixer.music.set_volume(0.5)
            pygame.mixer.music.play(1)
            for teacher in proctor_arnurka:
                teacher['speed']=0
                teacher['image'] = pygame.image.load('photo/arni_r.png')
            time.sleep(4)
    ...
    if clicked_desk and shkala < SCALE_WIDTH and update_scale:
        shkala += (5 / 30)
        progress = shkala / SCALE_WIDTH
    elif shkala > 0 and update_scale:
        shkala -= (2 / 30)
        progress = shkala / SCALE_WIDTH
